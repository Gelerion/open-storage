...
//        String s3Key = storage.mountedAt().resolve(file).asString();
//        s3.deleteObject(file.bucket(), file.key());
        s3.putObject(file.bucket(), file.key(), content);
//        storage.putObject(s3Key, content);
    }
...
/*    @Override
    public void upload(URL url) {
        String s3Key = storage.mountedAt().resolve(this.file).asString();
        storage.deleteObject(s3Key);
        //TODO: cleaner & faster way. this one hurts performance, once we have a time...
